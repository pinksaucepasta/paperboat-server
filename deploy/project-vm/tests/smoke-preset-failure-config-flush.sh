#!/usr/bin/env bash
set -Eeuo pipefail

root="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
tmp="$(mktemp -d)"
trap 'rm -rf "$tmp"' EXIT

bin="$tmp/bin"
events="$tmp/events.log"
mkdir -p "$bin" "$tmp/workspace/.git" "$tmp/runtime" "$tmp/logs"

cat > "$bin/prepare" <<'EOF'
#!/usr/bin/env bash
exit 0
EOF
cat > "$bin/config-sync" <<'EOF'
#!/usr/bin/env bash
set -Eeuo pipefail
case "${1:-}" in
  restore) printf 'config:restore\n' >> "$PAPERBOAT_TEST_EVENT_LOG" ;;
  daemon)
    trap 'printf "config:flushed\n" >> "$PAPERBOAT_TEST_EVENT_LOG"; exit 0' TERM
    while :; do sleep 1; done
    ;;
  save) printf 'config:fallback-save\n' >> "$PAPERBOAT_TEST_EVENT_LOG" ;;
esac
EOF
cat > "$bin/presets" <<'EOF'
#!/usr/bin/env bash
set -Eeuo pipefail
printf 'presets:modified-config\n' >> "$PAPERBOAT_TEST_EVENT_LOG"
exit 42
EOF
chmod +x "$bin"/*

export PAPERBOAT_TEST_EVENT_LOG="$events"
export PAPERBOAT_RUNTIME_DIR="$tmp/runtime"
export PAPERBOAT_LOG_DIR="$tmp/logs"
export PAPERBOAT_WORKSPACE="$tmp/workspace"
export PAPERBOAT_READINESS_FILE="$tmp/runtime/readiness.json"
export PAPERBOAT_PREPARE_WORKSPACE_COMMAND="$bin/prepare"
export PAPERBOAT_CONFIG_SYNC_COMMAND="$bin/config-sync"
export PAPERBOAT_PRESET_APPLY_COMMAND="$bin/presets"
export PAPERBOAT_PROJECT_ID=prj_preset_failure
export PAPERBOAT_REPOSITORY_URL=https://github.com/example/repo.git
export PAPERBOAT_AGENTUNNEL_SERVER_URL=https://agentunnel.example
export PAPERBOAT_AGENTUNNEL_CLIENT_ID=cli_preset_failure
export PAPERBOAT_AGENTUNNEL_TUNNEL_ID=tun_preset_failure
export AGENTUNNEL_MACHINE_TOKEN=token_preset_failure

set +e
"$root/bin/paperboat-entrypoint" >"$tmp/stdout" 2>"$tmp/stderr"
status=$?
set -e

if [ "$status" -ne 42 ]; then
  printf 'entrypoint status = %s, want 42\n' "$status" >&2
  exit 1
fi
grep -q 'presets:modified-config' "$events"
grep -Eq 'config:flushed|config:fallback-save' "$events"
preset_line="$(grep -n -m1 'presets:modified-config' "$events" | cut -d: -f1)"
flush_line="$(grep -n -m1 -E 'config:flushed|config:fallback-save' "$events" | cut -d: -f1)"
if [ "$preset_line" -ge "$flush_line" ]; then
  printf 'preset failure was not followed by a final config flush:\n' >&2
  cat "$events" >&2
  exit 1
fi
grep -q '"state":"failed"' "$PAPERBOAT_READINESS_FILE"
grep -q '"reason":"presets"' "$PAPERBOAT_READINESS_FILE"
