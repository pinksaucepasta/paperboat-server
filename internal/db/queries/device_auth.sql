...
-- name: GetActiveClientSessionForInstallation :one
SELECT user_id,client_id FROM cli_client_sessions WHERE id=$1 AND state='active';

-- name: GetActiveClientSessionForUserMachine :one
...
SELECT count(*) FROM cli_client_sessions WHERE user_id=sqlc.arg(user_id) AND (sqlc.arg(state_filter)::text='' OR state=sqlc.arg(state_filter));
...
FROM cli_client_sessions WHERE user_id=sqlc.arg(user_id) AND (sqlc.arg(state_filter)::text='' OR state=sqlc.arg(state_filter)) ORDER BY created_at DESC LIMIT sqlc.arg(row_limit) OFFSET sqlc.arg(row_offset);
