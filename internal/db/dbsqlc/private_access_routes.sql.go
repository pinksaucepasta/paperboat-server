...
// versions:
//   sqlc v1.30.0
// source: private_access_routes.sql
...
const listPrivateAccessCarrierAdmissionsForEdgeV1 = `-- name: ListPrivateAccessCarrierAdmissionsForEdgeV1 :many
WITH private_routes AS (
  ...
  WHERE a.edge_node_id = $2 AND a.edge_process_epoch = $3
    AND p.access_mode = 'private' AND p.terminal_state = 'active' AND p.lease_deadline > $4
    AND a.state = 'ready' AND a.expires_at > $4
    AND n.state = 'ready' AND n.ready = true
    AND n.last_heartbeat_at > $4 - interval '2 minutes'
  UNION ALL
  ...
  WHERE a.edge_node_id = $2 AND a.edge_process_epoch = $3
    ...
    AND s.state = 'ready' AND s.lease_deadline > $4
    AND n.state = 'ready' AND n.ready = true
    AND n.last_heartbeat_at > $4 - interval '2 minutes'
)
SELECT routes.account_id, routes.resource_kind, routes.resource_id, routes.tunnel_name, routes.route_name, routes.operation_id, routes.connector_id, routes.carrier_session_id, routes.route_id, routes.route_generation, routes.session_generation, routes.process_generation, routes.config_generation, routes.assignment_generation, routes.edge_node_id, routes.edge_process_epoch, routes.protocol, routes.hostname, routes.edge_endpoints, routes.expires_at, routes.tunnel_id, routes.carrier_connector_id, routes.assignment_id, routes.config_content_hash, routes.match_type, routes.wildcard_suffix, routes.edge_carrier_server_spki_sha256, routes.edge_carrier_server_certificate_chain_pem,
...
type ListPrivateAccessCarrierAdmissionsForEdgeV1Params struct {
	RowLimit         int32
	EdgeNodeID       string
...
type ListPrivateAccessCarrierAdmissionsForEdgeV1Row struct {
	AccountID                            string
	ResourceKind                         string
	ResourceID                           string
	TunnelName                           string
	...
	AccessorDeviceID                     string
	InstallationGeneration               int64
	AccessorPublicKey                    sql.NullString
}

func (q *Queries) ListPrivateAccessCarrierAdmissionsForEdgeV1(ctx context.Context, arg ListPrivateAccessCarrierAdmissionsForEdgeV1Params) ([]ListPrivateAccessCarrierAdmissionsForEdgeV1Row, error) {
	rows, err := q.db.Query(ctx, listPrivateAccessCarrierAdmissionsForEdgeV1,
		arg.RowLimit,
		arg.EdgeNodeID,
	...
	defer rows.Close()
	var items []ListPrivateAccessCarrierAdmissionsForEdgeV1Row
	for rows.Next() {
		var i ListPrivateAccessCarrierAdmissionsForEdgeV1Row
		if err := rows.Scan(
			&i.AccountID,
			&i.ResourceKind,
			&i.ResourceID,
			...
			&i.ExpiresAt,
			&i.TunnelID,
			&i.CarrierConnectorID,
			&i.AssignmentID,
			&i.ConfigContentHash,
			&i.MatchType,
			&i.WildcardSuffix,
			&i.EdgeCarrierServerSpkiSha256,
			&i.EdgeCarrierServerCertificateChainPem,
			&i.AccessorDeviceID,
...
const listPrivateAccessRoutesForMachineV1 = `-- name: ListPrivateAccessRoutesForMachineV1 :many
WITH private_routes AS (
  SELECT a.account_id,
...
type ListPrivateAccessRoutesForMachineV1Params struct {
	MachineID string
	RowLimit  int32
	AccountID string
	Now       time.Time
...
type ListPrivateAccessRoutesForMachineV1Row struct {
	AccountID                            string
	ResourceKind                         string
	ResourceID                           string
	TunnelName                           string
...
func (q *Queries) ListPrivateAccessRoutesForMachineV1(ctx context.Context, arg ListPrivateAccessRoutesForMachineV1Params) ([]ListPrivateAccessRoutesForMachineV1Row, error) {
	rows, err := q.db.Query(ctx, listPrivateAccessRoutesForMachineV1,
		arg.MachineID,
		arg.RowLimit,
		arg.AccountID,
		arg.Now,
	...
	defer rows.Close()
	var items []ListPrivateAccessRoutesForMachineV1Row
	for rows.Next() {
		var i ListPrivateAccessRoutesForMachineV1Row
		if err := rows.Scan(
			&i.AccountID,
			&i.ResourceKind,
			&i.ResourceID,
