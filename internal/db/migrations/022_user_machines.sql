...
CREATE TABLE user_machines (
  ...
  provider_route_route_id text,
  provider_route_client_id text,
  provider_route_http_base_url text,
  provider_route_websocket_base_url text,
...
CREATE UNIQUE INDEX user_machines_active_name
  ON user_machines(user_id, lower(display_name))
  WHERE deleted_at IS NULL;
CREATE INDEX user_machines_owner_state ON user_machines(user_id, state);
CREATE INDEX user_machines_provider_route_client ON user_machines(provider_route_client_id)
  WHERE provider_route_client_id IS NOT NULL;

CREATE TABLE user_machine_pairings (
  id text PRIMARY KEY,
...
CREATE INDEX user_machine_pairings_pending_code
  ON user_machine_pairings(user_code, expires_at) WHERE state = 'pending';

CREATE TABLE user_machine_bandwidth_periods (
  id text PRIMARY KEY,
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE (user_machine_id, period_start),
  CHECK (period_end > period_start)
);

CREATE TABLE user_machine_bandwidth_topups (
  id text PRIMARY KEY,
  user_id text NOT NULL REFERENCES users(id) ON DELETE CASCADE,
...
CREATE INDEX user_machine_bandwidth_topups_active
  ON user_machine_bandwidth_topups(user_id, created_at)
  WHERE state = 'active' AND remaining_bytes > 0;

-- +goose Down
DROP TABLE IF EXISTS user_machine_bandwidth_topups;
DROP TABLE IF EXISTS user_machine_bandwidth_periods;
DROP TABLE IF EXISTS user_machine_pairings;
