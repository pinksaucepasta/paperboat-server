...
CREATE OR REPLACE FUNCTION revoke_user_cli_client_sessions_on_status_change()
RETURNS trigger
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
	IF OLD.status = 'active' AND NEW.status <> 'active' THEN
		...
		WHERE cli_client_session_id IN (SELECT id FROM cli_client_sessions WHERE user_id = NEW.id);
...
DROP TRIGGER IF EXISTS trg_users_revoke_cli_client_sessions ON users;
CREATE TRIGGER trg_users_revoke_cli_client_sessions
AFTER UPDATE OF status ON users
FOR EACH ROW
WHEN (OLD.status IS DISTINCT FROM NEW.status)
EXECUTE FUNCTION revoke_user_cli_client_sessions_on_status_change();
