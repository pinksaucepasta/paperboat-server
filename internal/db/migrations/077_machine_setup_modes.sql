...
UPDATE user_machines SET setup_mode = CASE WHEN 'host' = ANY(setup_roles) THEN 'host' ELSE 'client' END;
UPDATE user_machines SET configured_capabilities = CASE setup_mode WHEN 'host' THEN ARRAY['file_receive','preview_launch','terminal_host','codex_host','session_host','keep_awake']::text[] WHEN 'client' THEN ARRAY['file_receive','preview_launch']::text[] END;
ALTER TABLE user_machines
