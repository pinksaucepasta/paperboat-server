...
UPDATE user_machines SET configured_capabilities = CASE setup_mode WHEN 'host' THEN ARRAY['file_receive','preview_launch','terminal_host','codex_host','session_host','keep_awake']::text[] WHEN 'receive' THEN ARRAY['file_receive','preview_launch']::text[] ELSE '{}'::text[] END;
ALTER TABLE user_machines
  ADD CONSTRAINT user_machines_setup_mode_check CHECK (setup_mode IN ('receive','session','host')),
