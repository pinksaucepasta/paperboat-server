-- +goose Up
CREATE TABLE user_machine_entitlements (
  id text PRIMARY KEY,
...
CREATE INDEX user_machine_entitlements_active ON user_machine_entitlements(user_id, current_period_end)
  WHERE state IN ('trialing','active');

-- +goose Down
DROP TABLE IF EXISTS user_machine_entitlements;
