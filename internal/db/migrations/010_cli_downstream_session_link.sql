...
ADD COLUMN IF NOT EXISTS cli_client_session_id text REFERENCES cli_client_sessions(id);

CREATE INDEX IF NOT EXISTS idx_access_sessions_client_session
ON access_sessions(cli_client_session_id)
...
DROP TRIGGER IF EXISTS trg_cli_client_sessions_revoke_access ON cli_client_sessions;
CREATE TRIGGER trg_cli_client_sessions_revoke_access
