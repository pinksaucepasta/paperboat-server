...
func TestPreviewAttachmentV1SchemaVectors(t *testing.T) {
	...
	fixtures, err := os.Open(filepath.Join(familyRoot, "fixtures", "attachment.ndjson"))
	if err != nil {
		t.Fatal(err)
	}
	...
	for scanner.Scan() {
		...
		if vector.Valid {
			seenValid++
			if !result.Valid() {
				t.Errorf("%s: valid attachment message rejected: %v", vector.Case, result.Errors())
			}
			continue
		}
		seenInvalid++
		...
		if result.Valid() {
			t.Errorf("%s: invalid attachment message accepted", vector.Case)
		}
	}
	if err := scanner.Err(); err != nil {
		t.Fatal(err)
	}
	if seenValid != 5 || seenInvalid != 5 {
