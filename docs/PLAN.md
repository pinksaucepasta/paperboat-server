...
| 9 | agentunnel pre-connect brokering and access descriptors | In progress | Codex | Mint/exchange, stable VM papercode identity provisioning, durable downstream linkage, signed revocation including enforcement retry, and key-overlap rollback tests are implemented. Real-provider evidence is deferred to release validation; the staged-image descriptor remains separate implementation work. |
...
- [x] Invalidate actual papercode terminal/file sessions when a CLI family is revoked through
  the signed, replay-guarded control-plane endpoint with durable retry for enforcement paths.
