...
### Native relay lifecycle v1 (Task 13)

`POST /v1/relay/nodes/start` and `POST /v1/relay/nodes/observe` use the existing
`bearerEdgeControl` authenticated control-plane handler. They grant no new account,
region, endpoint or peer-relay service authority: startup accepts only an existing
operator-provisioned relay registry row. Start compares `expected_generation`, installs
a fresh `process_epoch`, increments node generation and initially publishes not-ready.
The same pending start is idempotent after a lost response. A competing generation or
stale process heartbeat fails with `409 relay_node_fenced`; unknown nodes cannot register.

Observations bind exact node ID, node generation and process epoch, report readiness,
drain and measured active connection count, and carry at most 256 distinct admitted
`account_id`, `endpoint_id`, `generation` subjects. Counts cannot exceed the preprovisioned
capacity (at most 256). Server receipt time controls freshness; client timestamps cannot
extend authority. Drain is irreversible within a process epoch. A new startup is required
after process fencing or registry expiration.

An observation renews the regional registry for at most 60 seconds, retaining the existing
15-second health/capacity freshness checks. The response `expires_at` is a separate
15-second control lease. The command observes every three seconds, preserves existing
connections through transient control loss only within that lease, and closes them on
expiration. This leaves room for 15-second relay credential renewal without issuing
artificially short grants. Stopped processes publish drain and release listeners/workers.

Responses include only relevant newer endpoint revocation generations. Native authority
mutation triggers update `relay_revocation_generation`; ordinary configuration refresh
increments its delivery generation without revoking compatible live grants. The relay
applies newer floors using `Server.Revoke`, closes affected connections and rejects old
admission. Unknown or newly policy-excluded subjects are fenced above their observed
generation. Observations contain routing identifiers and numeric authority/capacity
metadata only, never tokens, private keys, application content or user file payloads.

When the registry advertises `peer_relay`, the lease includes its exact public
`peer_relay` service descriptor. Command startup and health observation require the
configured service to match that descriptor; missing, unexpected or mismatched UDP
service identities cannot publish readiness. DERP-only nodes omit that field.
