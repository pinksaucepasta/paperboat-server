...
func run(ctx context.Context, args []string, output *os.File) error {
	...
	if len(args) == 6 {
		...
		request := httptest.NewRequest(http.MethodPost, "/v1/edge/routes/observations", bytes.NewBufferString(`{"edge_node_id":"`+nodeID2+`","routes":[]}`))
...
func resetAndSeed(ctx context.Context, store *db.DB) error {
	statements := []string{
		...
		`INSERT INTO paperboat.control_routes (id,environment_id,kind,public_host,target_host,target_port) VALUES ('route_control_conformance','env_control_conformance','runtime_https_wss','control-conformance.runtime.example.test','127.0.0.1',8080)`,
	}
